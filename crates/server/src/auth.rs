...
use crate::openapi::{RegisterRequest, LoginRequest};
...
pub struct ServerState {
    ...
    pub admin_store: std::sync::Arc<service::admin_kv_store::ApiKeysStore>,
...
#[utoipa::path(post, path = "/auth/register", tag = "auth", request_body = RegisterRequest, responses((status = 200, description = "Registered"), (status = 400, description = "Bad Request"), (status = 409, description = "Conflict")))]
...
#[utoipa::path(post, path = "/auth/login", tag = "auth", request_body = LoginRequest, responses((status = 200, description = "Logged In"), (status = 401, description = "Unauthorized")))]
